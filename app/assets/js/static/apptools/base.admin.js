...
(function() {
  var AppException, AppTools, AppToolsCollection, AppToolsException, AppToolsModel, AppToolsRouter, AppToolsView, BBQController, BlogManagerAPI, ContentManagerAPI, CoreAPI, CoreAdminAPI, CoreAgentAPI, CoreDevAPI, CoreDispatchAPI, CoreEventsAPI, CoreException, CoreInterface, CoreModelAPI, CoreObject, CorePushAPI, CoreRPCAPI, CoreRenderAPI, CoreStorageAPI, CoreUserAPI, CoreWidget, CoreWidgetAPI, Expand, Find, Milk, Modal, ModalAPI, Openfire, OpenfireController, OpenfireException, OpenfireObject, PageManagerAPI, Parse, PushDriver, QueryDriver, RPCAPI, RPCDriver, RPCRequest, RPCResponse, RenderDriver, SiteManagerAPI, StorageDriver, TemplateCache, Util,
    __slice = [].slice,
    ...
    function ModalAPI(apptools, widget, window) {
      ...
      this.create = function(target, trigger, options) {
        ...
        if (target.hasAttribute('data-options')) {
          options = JSON.parse(target.getAttribute('data-options'));
        }
  ...
  BBQController = (function() {
    ...
    function BBQController(openfire, window) {
      var _this = this;
      this._init = function() {
        _this.initBbqInlines();
      };
    }

    BBQController.prototype.initBbqInlines = function() {
      var _this;
      _this = this;
      ...
      $('.save-new-inline').click(function() {
        ...
        if (request) {
          return _this.fulfillBbqRequest(request, 'Failed to something.');
        }
      });
      $('.delete-object').click(function() {
        var request, toDelete;
        request = void 0;
        toDelete = void 0;
        toDelete = this.id.match(/delete-(\w+)-(\w+)/);
        request = null;
        switch (toDelete[1]) {
          case 'category':
        ...
        if (request) {
          return _this.fulfillBbqRequest(request, 'Failed to delete ' + toDelete[1]);
        }
      });
      $('.start-edit-inline').click(function() {
        var editType, slug, toEdit;
        toEdit = this.id.match(/start-edit-(\w+)-(\w+)/);
        editType = toEdit[1];
        slug = toEdit[2];
        ...
        return $('#' + editType + '-inputs-' + slug).show();
      });
      $('.cancel-edit-inline').click(function() {
        var editType, slug, toEdit;
        toEdit = this.id.match(/cancel-edit-(\w+)-(\w+)/);
        editType = toEdit[1];
        slug = toEdit[2];
        ...
        return $('#' + editType + '-inputs-' + slug).hide();
      });
      return $('.save-edit-inline').click(function() {
        var categoryDict, request, toSave;
        toSave = this.id.match(/save-edit-(\w+)-(\w+)/);
        switch (toSave[1]) {
          case 'category':
            categoryDict = {
        ...
        return _this.fulfillBbqRequest(request, 'Failed to edit something.');
      });
    };

    BBQController.prototype.fulfillBbqRequest = function(request, errMsg) {
