...
(function() {
  var AppException, AppTools, AppToolsCollection, AppToolsException, AppToolsModel, AppToolsRouter, AppToolsView, BlogManagerAPI, ContentManagerAPI, CoreAPI, CoreAdminAPI, CoreAgentAPI, CoreDevAPI, CoreDispatchAPI, CoreEventsAPI, CoreException, CoreInterface, CoreModelAPI, CoreObject, CorePushAPI, CoreRPCAPI, CoreRenderAPI, CoreStorageAPI, CoreUserAPI, CoreWidget, CoreWidgetAPI, Expand, Find, Milk, Modal, ModalAPI, PageManagerAPI, Parse, PushDriver, QueryDriver, RPCAPI, RPCDriver, RPCRequest, RPCResponse, RenderDriver, Scroller, ScrollerAPI, SiteManagerAPI, StorageDriver, TemplateCache, Util,
    __slice = [].slice,
    ...
    Util.prototype.bind = function(element, event, fn, prop) {
      ...
      if (this.is_array(element)) {
        ...
        for (_i = 0, _len = element.length; _i < _len; _i++) {
          el = element[_i];
          return this.bind(el, event, fn, prop);
        }
      }
      if (this.is_raw_object(event)) {
        for (ev in event) {
          func = event[ev];
          return this.bind(element, ev, func, prop);
        }
      }
      console.log('[Util]: Binding ' + fn + ' function to ' + event + ' event on ' + (e = element.getAttribute('id') ? element.getAttribute('id') : element.className) + '.');
  ...
  ModalAPI = (function(_super) {
    ...
    function ModalAPI(apptools, widget, window) {
      ...
      this.create = function(target, trigger, options) {
        ...
        _this._state.modals_by_id[modal._state.element_id] = _this._state.next;
        _this._state.modals.push(modal);
        _this._state.next++;
        return modal._init();
      };
    ...
    function Modal(target, trigger) {
      ...
      this._state = {
        ...
        config: {
          ...
          template: ['<div id="modal-dialog" style="opacity: 0;" class="fixed dropshadow modal">', '<div id="modal-fade" style="opacity: 0">', '<div id="modal-content">&nbsp;</div>', '<span class="modal-ui" class="absolute">', '<span id="modal-title" class="absolute"></span>', '<span id="modal-close" class="absolute">X</span>', '</span>', '</div>', '</div>'].join('\n'),
          rounded: true
        }
      };
      ...
      this.make = function() {
        var close_x, content, d, dialog, doc, fade, id, prop, range, template, title, val, _ref;
      ...
      this.open = function() {
        var dialog, dialog_animation, final, o, overlay_animation;
        dialog = Util.get(_this._state.element_id);
        _this._state.active = true;
        o = _this.prepare_overlay('modal');
        _this._state.overlay_id = o.getAttribute('id');
        document.body.appendChild(o);
        dialog_animation = overlay_animation = _this.animation;
        dialog_animation.complete = function() {
          var c;
          return Util.bind(c = Util.get('modal-close-' + _this._state.cached_id), 'mousedown', _this.close);
        };
        overlay_animation.complete = function() {
          return Util.bind(o, 'mousedown', _this.close);
        };
        final = _this.calc();
        final.opacity = 1;
      ...
      this.close = function() {
        var dialog, fade_anim, final_anim, midpoint, midpoint_anim, overlay, overlay_anim;
        _this._state.active = false;
        overlay = Util.get(_this._state.overlay_id);
        dialog = Util.get(_this._state.element_id);
        midpoint = {
        ...
        fade_anim = midpoint_anim = final_anim = overlay_anim = _this.animation;
        overlay_anim.duration = fade_anim.duration = midpoint_anim.duration = 300;
        final_anim.duration = 250;
        overlay_anim.complete = function() {
          document.removeChild(overlay);
          return document.removeChild(dialog);
        };
        midpoint_anim.complete = function() {
          $(dialog).animate({
            opacity: 0
          }, finalAnim);
        ...
        Util.get('modal-content-' + _this._state.cached_id).style.overflow = 'hidden';
        $('modal-fade-' + _this._state.cached_id).animate({
          opacity: 0
        }, fade_anim);
        return dialog;
      };
  ...
  Scroller = (function(_super) {
    ...
    function Scroller(target, options) {
      ...
      this.jump = function(pane) {
        ...
        animation.complete = function() {
          ...
          return _this.state.current_pane = pane.getAttribute('id');
        };
        frameO = _this.util.getOffset(target);
        paneO = _this.util.getOffset(pane);
        if (_this.config.axis === 'horizontal') {
    ...
    Scroller.prototype._init = function() {
      ...
      return $.apptools.events.trigger('SCROLLER_READY', this);
    };
  ...
  ScrollerAPI = (function(_super) {
    ...
    function ScrollerAPI(apptools, widget, window) {
      ...
      this.create = function(target, options) {
        ...
        _this.state.scrollers_by_id[scroller.state.el.getAttribute('id')] = _this.state.next;
        _this.state.scrollers.push(scroller);
        return scroller;
      };
      this.destroy = function(scroller) {
        var id;
        id = scroller.state.el.getAttribute('id');
        _this.state.scrollers.splice(_this.state.scrollers_by_id[id], 1);
        delete _this.state.scrollers_by_id[id];
        return scroller;
      };
      this.enable = function(scroller) {
        _this.prime(scroller.state.panes, scroller.jump);
        return scroller;
      };
      this.disable = function(scroller) {
        _this.unprime(scroller.state.panes);
        return scroller;
      };
    }

    ScrollerAPI.prototype._init = function(apptools) {
      var scroller, scrollers, _fn, _i, _len,
        _this = this;
      scrollers = this.util.get('scroller');
      _fn = function(scroller) {
        var axis;
        axis = scroller.getAttribute('data-axis');
        scroller = axis != null ? _this.create(scroller, {
          axis: axis
        }) : _this.create(scroller);
        return scroller = _this.enable(scroller);
      };
      for (_i = 0, _len = scrollers.length; _i < _len; _i++) {
        scroller = scrollers[_i];
        _fn(scroller);
      }
      return apptools.events.trigger('SCROLLER_API_READY', this);
    };
  ...
  this.__apptools_preinit.abstract_base_classes.push(ColorPicker);

  this.__apptools_preinit.abstract_base_classes.push(ColorPickerAPI);

  this.__apptools_preinit.deferred_core_modules.push({
    module: ColorPickerAPI,
    "package": 'widgets'
  });
