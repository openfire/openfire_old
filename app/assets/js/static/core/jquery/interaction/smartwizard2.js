    ...
    $.fn.smartWizard = function(action) {
        ...
        return this.each(function(){
                ...
                function init(){
                  ...
                  elmActionBar = $('<div></div>').addClass("action-bar");
                  elmStepContainer = $('<div></div>').addClass("step-container");
                  btNext = $('<a>'+options.labelNext+'</a>').attr("href","#").addClass("button-next");
                  btPrevious = $('<a>'+options.labelPrevious+'</a>').attr("href","#").addClass("button-previous");
                  btFinish = $('<a>'+options.labelFinish+'</a>').attr("href","#").addClass("button-finish");
                ...
                function adjustButton(){
                  if(!options.cycleSteps){
                    if(0 >= curStepIdx){
                      $(btPrevious).addClass("button-disabled");
                    }else{
                      $(btPrevious).removeClass("button-disabled");
                    }
                    if((steps.length-1) <= curStepIdx){
                      $(btNext).addClass("button-disabled");
                    }else{
                      $(btNext).removeClass("button-disabled");
                    }
                  }
                  // Finish Button
                  if(!steps.hasClass('disabled') || options.enableFinishButton){
                    $(btFinish).removeClass("button-disabled");
                  }else{
                    $(btFinish).addClass("button-disabled");
                  }
                }
