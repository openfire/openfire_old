...
def run(environment):
        ...
        for l_provider in environment.getattr(environment.getattr(environment.getattr(l_security_config, 'authentication'), 'federation'), 'providers'):
            ...
            if environment.getattr(l_provider, 'enabled'):
                ...
                yield u'<button %s>Login with %s</button>\n\t\t\t' % (
                    t_2(context.eval_ctx, {'class': context.call(environment.getattr(' ', 'join'), [l_disabled, 'zocial', environment.getattr(l_provider, 'name'), 'logon', 'button']), 'data-provider-name': environment.getattr(l_provider, 'name'), 'data-provider-label': environment.getattr(l_provider, 'label'), 'data-provider-mode': environment.getattr(l_provider, 'mode'), 'disabled': (None if (not l_disabled) else environment.undefined("the inline if-expression on line 32 in '/source/snippets/login_box.html' evaluated to false and no else section was defined.")), 'data-action': context.call(l_link, 'auth/login-with', provider=environment.getattr(l_provider, 'name')), 'onclick': t_1(("javascript:window._initExtAuth('%(name)s', '%(mode)s');" % l_provider))}), 
    ...
    blocks = {}
    debug_info = '21=15&22=22&23=34&24=37&25=39&35=46&36=47&44=52&45=61'
    return locals()