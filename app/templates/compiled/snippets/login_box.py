...
def run(environment):
    ...
    def root(context, environment=environment):
        ...
        for l_provider in environment.getattr(environment.getattr(environment.getattr(l_security_config, 'authentication'), 'federation'), 'providers'):
            ...
            if environment.getattr(l_provider, 'enabled'):
                ...
                yield u'<button %s>Login with %s</button>\n\t\t\t' % (
                    t_2(context.eval_ctx, {'id': ('%s-federated-logon-trigger' % environment.getattr(l_provider, 'name')), 'class': context.call(environment.getattr(' ', 'join'), [l_disabled, 'zocial', environment.getattr(l_provider, 'name'), 'logon', 'button']), 'data-provider-name': environment.getattr(l_provider, 'name'), 'data-provider-label': environment.getattr(l_provider, 'label'), 'data-provider-mode': environment.getattr(l_provider, 'mode'), 'disabled': (None if (not l_disabled) else 'disabled'), 'data-action': context.call(l_link, 'auth/login-with', provider=environment.getattr(l_provider, 'name')), 'onclick': t_1(("javascript:window._initExtAuth('%(name)s', '%(mode)s');" % l_provider))}), 
        ...
        if (not l_error):
            if 0: yield None
            yield u'<h4>Sign in using your openfire account</h4>'
        else:
            if 0: yield None
            yield u'<h4 class="woops">%s</h4>' % (
                t_1(l_error), 
    ...
    blocks = {}
    debug_info = '19=15&20=22&21=34&22=37&23=39&34=46&35=47&43=52&44=61'
    return locals()